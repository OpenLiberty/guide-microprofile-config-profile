...
Deploying the application to a Kubernetes environment using the Open Liberty Operator is an optional step in this guide. For a comprehensive walkthrough on deploying to Kubernetes with the Open Liberty Operator, refer to the https://openliberty.io/guides/openliberty-operator-intro.html[Deploying a microservice to Kubernetes using Open Liberty Operator^] guide.
...
The [hotspot file=0]`deploy.yaml` file configures the deployment of the `system` and `query` microservices using the Open Liberty Operator. It sets the environment variables [hotspot=default.username file=0]`DEFAULT_USERNAME` and [hotspot=default.password file=0]`DEFAULT_PASSWORD` for the [hotspot=system file=0]`system` microservice, and [hotspot=system.user file=0]`SYSTEM_USER` and [hotspot=system.password file=0]`SYSTEM_PASSWORD` for the [hotspot=query file=0]`query` service, using the `sys-app-credentials` Kubernetes secrets.
