...
Deploy the `system` service in the testing environment and access it with the `query` service, which is running in https://openliberty.io/docs/latest/development-mode.html[dev mode^].
...
During testing, the Liberty configurations from the [hotspot=testing file=0]`testing` profile in the [hotspot file=0]`system/pom.xml` file are used for running the `system` service. You can find out that the `system` service runs on port [hotspot=test.port file=0]`9082` with context root [hotspot=test.context.root file=0]`system/test`, and a basic user registry is set up with username [hotspot=test.username file=0]`bob` and password [hotspot=test.password file=0]`bobpwd` to authorize access to resources from other services. Note that the `basicRegistry` element is a simple case for learning purposes. For more information on different user registries, see the https://openliberty.io/docs/latest/user-registries-application-security.html[User registries documentation^].

Next, open another command-line session.

When you run Open Liberty in dev mode, known as dev mode, dev mode listens for file changes and automatically recompiles and deploys your updates whenever you save a new change. Run the following commands to navigate to the `query` directory and start the `query` service in dev mode:
...
During development, the Liberty configurations from the [hotspot=development file=0]`development` profile in the [hotspot file=0]`system/pom.xml` file are used for running the `system` service. The `system` service now runs on port [hotspot=port file=0]`9081` with context root [hotspot=context.root file=0]`system/dev`, and the credentials for the user registry are updated with username [hotspot=username file=0]`alice` and password [hotspot=password file=0]`alicepwd` for authentication to the service. 
...
Once you have updated the properties, stop the `query` service by pressing `CTRL+C` in the command-line session where you ran the `query` service.
...
Update the `microprofile-config.properties` file by removing the `%development.\*` properties and setting the [hotspot=system file=2]`system.*` properties to their default production values from the [hotspot=defaultConfig file=3]`system/server.xml` configuration file.
...
When you are done checking out the application in `development` environment, exit dev mode by pressing `CTRL+C` in the command-line sessions where you ran the `system` and `query` services.
