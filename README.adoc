...
Before deploying, build and containerize the application at the `start` directory.
...
The Maven `clean` and `package` goals can clean the `target` directories and build the `.war` application files from scratch.
...
The Docker `build` commands package the `.war` file of the [hotspot=config hotspot=war file=1]`system` and [hotspot=config hotspot=war file=2]`query` microservices with their default configuration into your Docker images.
