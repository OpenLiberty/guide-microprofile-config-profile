...
Run the following command by replacing the `[username]` and `[password]` parameters. The `deployApp.sh` will  [hotspot=build file=0]`build` and [hotspot=start file=0]`start` the services with [hotspot=systemProd hotspot=queryProd file=0]`prod` profile.
