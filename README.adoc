...
You'll learn how to provide environment-specific configurations by using MicroProfile Config's configuration profile feature. You'll create configuration profiles using individual profile-specific configuration properties and profile-specific default `microprofile-config.properties` configuration sources, working with the MicroProfile Config API.
...
The application that you will work with is a `query` service, which fetches information about the running JVM from a `system` microservice. Given the differences in setup between development, testing, and production environments, you'll use MicroProfile Config's configuration profiles to externalize and manage the configurations across the different environments.
