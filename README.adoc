...
Ensuring security in the production environment is essential. In the previous sections, you have learnt how to externalize credentials and other properties for accessing the `system` service by using MicroProfile Config, making the application more adaptable to different environments without changing code or configuration files. In the this section, you'll learn how to make use of Kubernetes secrets to provide the credentials and how to pass them to the `query` service through the MicroProfile Config.
