...
When you run Open Liberty in dev mode, known as dev mode, dev mode listens for file changes and automatically recompiles and deploys your updates whenever you save a new change. 

Open a command-line session and run the following commands to navigate to the `system` directory and start the `system` service in `dev` mode:
...
Open a command-line session and run the following commands to navigate to the `query` directory and start the `query` service in `dev` mode:
...
Test the connection between the `query` service and the `system` service under the current development environment by pointing your browser to the http://localhost:9085/query/systems/localhost URL. The `query` service returns the message: `{"fail":"Failed to reach the client localhost."}`. This is because the `query` service doesn't have a specific `dev` configuration profile. As a result, it falls back to using the default `microprofile-config.properties` config source.

To communicate with the `system` service, which is in its development phase, the `query` service needs to establish a `dev` configuration profile.
