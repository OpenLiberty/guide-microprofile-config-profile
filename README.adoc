...
The development environment is a foundational stage where developers experiment, debug, and refine their code. This ensures an application's reliability before progressing to further stages.
...
In software, there are different setups for tasks like development, testing, and production. Making dev the starting setup helps avoid mistakes and makes work smoother. 
...
MicroProfile Config's configuration profile feature allows for the supply of configurations for different environments while only a single profile is active. The active profile is set using the `mp.config.profile` property, which acts as a unique identifier for each configuration profile and can be set in any of the https://openliberty.io/guides/microprofile-config-intro.html#background-concepts[ConfigSources] or during application startup. When a profile is active, its associated configuration properties are used. For the `query` service, the `mp.config.profile` is set to [hotspot=mp.config.profile file=1]`dev` in the Maven `pom.xml` as the default configuration profile.
...
In the DevOps process, the production environment must be both secure. One way to ensure security is by externalizing sensitive credentials, keeping them safe from potential threats.
