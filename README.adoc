...
In software, there are different setups for tasks like development, testing, and going live. Making dev the starting setup is a practical approach. It helps smooth out the workflow by ensuring immediate access to development-specific resources without any additional setup.
...
In the dev environment, the configurations from the [hotspot=developmentProperties file=0]`dev` profile in the [hotspot file=0]`system/pom.xml` file are used for running the `system` service. The `system` service runs on port [hotspot=port file=0]`9081` using the context root [hotspot=context.root file=0]`system/dev`. It uses a basic user registry with username [hotspot=username file=0]`alice` and password [hotspot=password file=0]`alicepwd` for resource authorization. Note that the `basicRegistry` element is a simple case for learning purposes. For more information on user registries, see the https://openliberty.io/docs/latest/user-registries-application-security.html[User registries documentation^].

Navigate to the [hotspot file=2]`query/src/main/resources/META-INF/microprofile-config.properties` local configuration file to check some static configuration in the `query` service. This file is the default configuration source for an application using MicroProfile Config and contains the required properties to access the `prod` profile of the `system` service.

Point your browser to the http://localhost:9085/query/systems/localhost URL. The `query` service returns the message: `{"fail":"Failed to reach the client localhost."}`. This is because the current `query` service lacks a `dev` configuration profile, leading it to use the default configurations in the [hotspot file=2]`microprofile-config.properties` file when making a request to the `system` service. For proper communication with the developing `system` service, the `query` service should set up a `dev` configuration profile.
...
// =================================================================================================
// Deploying and running the application in prod environment
// =================================================================================================
== Deploying and running the application in prod environment

Now that the application is ready to be deployed.

In the DevOps process, the prod environment must be secure. One way to ensure security is by externalizing sensitive details like usernames and passwords. This keeps these critical credentials away from direct embedding in the code, keeping them safe from potential threats.

// file 0
scripts/deployApp.sh|.bat
[source, text, linenums, role='code_column']
----
include::finish/scripts/deployApp.sh[]
----
...
The [hotspot=credentials file=1]`server.xml` and the [hotspot=credentials file=2]`microprofile-config.properties` files provide the default credentials for the `system` and `query` services. While these default settings make development easier, they aren't ideal for production.

For simplicity, in this guide, the [hotspot file=0]`deployApp.sh` script is used to simulate the production deployment. To enhance security, the `deployApp.sh` script allows you to input the username and password during deployment. Through setting the system properities by the [hotspot=systemCredential hotspot=queryCredential file=0]`-D` options, both `system` and `query` services start with the secured credentials. To learn how to externalize configuration your application in Kubernetes, you can read the https://openliberty.io/guides/kubernetes-microprofile-config.html[Configuring microservices running in Kubernetes^] guide.

Run the following command by replacing the `[username]` and `[password]` parameters. The `deployApp.sh` will  [hotspot=build file=0]`build` and [hotspot=start file=0]`start` the services with [hotspot=systemProd hotspot=queryCredential file=0]`prod` profile.
...
[role='command']
```
./scripts/deployApp.sh [username] [password]
```
--
...
[role='command']
```
scripts\deployApp.bat [username] [password]
```
--
...
After you finish checking out the application, stop the Open Liberty instances by running the following goals in the same command-line session where you deployed the `system` and `query` services:
