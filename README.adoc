...
Ensuring security in the production environment is essential. In the previous sections, you have learnt how to externalize the credentials and other properties for accessing the `system` service by using the MicroProfile Config that makes the application more adaptable to different environments without changing code or configuiration files.  In the this section, you'll learn how to make use of Kubernetes secret to provide the credentials and how to pass them to the `query` service through the MicroProfile Config.
...
After building the images, you can create a Kubernetes secret for storing sensitive data such as credentials. To learn more how to manage the secrets, visit the https://kubernetes.io/docs/tasks/configmap-secret/managing-secret-using-kubectl[Managing Secrets using kubectl^] documentation 
...
If you want to override the other property, you can specify it through the `env` in the `deploy.yaml`. For example, set in the [hotspot file=5 hotspot=context.root]`CONTEXT_ROOT` in the `system` deployment and [hotspot file=5 hotspot=system.contextRoot]`SYSTEM_CONTEXTROOT` in the `query` deployment.
