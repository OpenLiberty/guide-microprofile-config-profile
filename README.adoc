...
Test the application under the `testing` environment by running the following script that contains different Maven goals to [hotspot=build file=0]`build`, [hotspot=start file=0]`start`, [hotspot=test file=0]`test`, and [hotspot=stop file=0]`stop` the services. Note that the `system` service starts with the Maven [hotspot=start-system file=0]`testing` profile and the `query` service uses the [hotspot=start-query file=0]`testing` MicroProfile Config profile.
