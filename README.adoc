...
Ensuring security in the production environment is essential. In the previous sections, you have learn how to externalize the credentials and other properties for accessing the `system` service by using the MicroProfile Config that makes the application more adaptable to different environments without changing code or configuiration files. MicroProfile Config also makes the deployment more secure. In the this section, you'll learn how to make use of Kubernetes secret to provide the credentials and how to pass them to the `query` service.
...
If you want to override the other property, you can specify it through the `env` in the `deploy.yaml` as the following example:
[source, role="no_copy"]
----
env:
