...
If you want to override the other property, you can specify it through the `env` in the `deploy.yaml`. For example, set in the [hotspot=context.root file=5]`CONTEXT_ROOT` in the `system` deployment and [hotspot=system.contextRoot file=5]`SYSTEM_CONTEXTROOT` in the `query` deployment.
